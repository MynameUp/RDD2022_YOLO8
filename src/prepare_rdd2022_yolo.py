...
#     os.makedirs(path, exist_ok=True)
#
# def process_split(src_root, dst_root, split="train"):
#     img_out_dir = os.path.join(dst_root, "images", split)
#     label_out_dir = os.path.join(dst_root, "labels", split)
#
#     ensure_dir(img_out_dir)
...
#
#     for country in COUNTRIES:
#         img_dir = os.path.join(src_root, country, split, "images")
#         xml_dir = os.path.join(src_root, country, split, "annotations", "xmls")
#
#         if not os.path.exists(img_dir) or not os.path.exists(xml_dir):
#             print(f"[Warning] 跳过不存在路径: {country} {split}")
#             continue
#
#         xml_files = glob(os.path.join(xml_dir, "*.xml"))
#
#         for xml_file in tqdm(xml_files, desc=f"{country}-{split}"):
#             base_name = os.path.splitext(os.path.basename(xml_file))[0]
#
#             # 找对应图片，可能是 jpg/png
#             image_file = None
...
#     dst_root = "../datasets/rdd2022_yolo"  # 转换后的 YOLO 数据路径
#
#     process_split(src_root, dst_root, split="train")
#     process_split(src_root, dst_root, split="val")
#
#     print("RDD2022 转换为 YOLO 格式完成！")
#
# if __name__ == "__main__":
